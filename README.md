...
- Подтверждение аномальных цен повторными проверками и несколькими источниками.
- Отдельные ленты Chrome-парсера и Error Fare, фильтры и график истории цены.
- Heartbeat Chrome-расширения и защита Telegram от повторных уведомлений.
...
- `GET /api/providers/browser` - состояние и последние результаты Chrome-парсера.
- `GET /api/results` - фильтруемая лента всех результатов.
- `GET /api/error-fares` - лента кандидатов Error Fare.
- `GET /api/price-chart` - данные графика цены по маршруту.
...
По умолчанию расширение проверяет треки с окном не больше 7 дней каждые 5 минут. При проверке Chrome создает и закрывает фоновые вкладки авиакомпаний. Расширение отправляет heartbeat на сервер раз в минуту. Если обнаружена аномальная цена, она перепроверяется ещё два раза с интервалом 45 секунд. Если Аэрофлот или S7 показывает CAPTCHA/антибот-проверку, её нужно один раз пройти вручную в обычной вкладке Chrome.
# flightparser
